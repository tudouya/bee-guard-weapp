...
Page({
  ...
  choose() { wx.chooseImage({ count: 3, sizeType: ['compressed'], success: (res)=>{ const list=(this.data.images||[]).concat(res.tempFilePaths||[]); this.setData({ images: list.slice(0,3) }); } }); },
  ...
  _doSubmit() { if(!this.data.form.title){ wx.showToast({ title:'请输入标题', icon:'none' }); return;} wx.showToast({ title:'已提交审核', icon:'success' }); setTimeout(()=>wx.navigateBack(), 600); }
});
